...
export default function CarouselManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/carousel/${editingId}`, form)
        showToast('更新成功')
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                {form.image_url && (
                  <img src={form.image_url} alt="預覽" className="h-16 rounded border object-cover" />
                )}
              </div>
            </div>
            ...
            <div key={slide.id} className="bg-white rounded shadow-sm overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button onClick={() => handleDelete(slide.id)} className="text-red-400 hover:text-red-600 text-xs">刪除</button>
                </div>
              </div>
            </div>
          ))
